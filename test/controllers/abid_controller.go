...
import (
	...
	cachev1alpha1 "test/api/v1alpha1"
)

// AbidReconciler reconciles a Abid object
type AbidReconciler struct {
	client.Client
	Scheme *runtime.Scheme
	Log    logr.Logger
}

//+kubebuilder:rbac:groups=cache.example.com,resources=abids,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=cache.example.com,resources=abids/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=cache.example.com,resources=abids/finalizers,verbs=update

// Reconcile is part of the main kubernetes reconciliation loop which aims to
// move the current state of the cluster closer to the desired state.
// TODO(user): Modify the Reconcile function to compare the state specified by
// the Abid object against the actual cluster state, and then
// perform operations to make the cluster state reflect the state specified by
// the user.
//
// For more details, check Reconcile and its Result here:
// - https://pkg.go.dev/sigs.k8s.io/controller-runtime@v0.10.0/pkg/reconcile
func (r *AbidReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	_ = log.FromContext(ctx)

	// TODO(user): your logic here
	r.Log = ctrl.Log.WithValues("Abid", req.NamespacedName)

	abid := &cachev1alpha1.Abid{}
...
func (r *AbidReconciler) deploymentForAbid(m *cachev1alpha1.Abid) *appsv1.Deployment {
...
func (r *AbidReconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&cachev1alpha1.Abid{}).
		Complete(r)
}
